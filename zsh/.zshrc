...
plugins=(
  ...
  # zsh-autosuggestions
  colored-man-pages
  # third-party, cloned into $ZSH_CUSTOM/plugins by install.sh
  auto-ls
...
# auto-ls and zsh-syntax-highlighting are loaded via plugins=() above
...
# 010 Editor: set ZERO10_EDITOR to the binary if it is not on PATH
if [ -n "$ZERO10_EDITOR" ] && [ -x "$ZERO10_EDITOR" ]; then
    alias 010="$ZERO10_EDITOR"
    alias 010editor="$ZERO10_EDITOR"
elif command -v 010editor >/dev/null 2>&1; then
    alias 010='010editor'
fi
...
#########################
#  TOOL PATHS (machine-specific; every one is optional)

# Ghidra: point GHIDRA_INSTALL_DIR at the unpacked release, or drop one
# under ~/Applications and the newest ghidra_*_PUBLIC dir is picked up.
if [ -z "$GHIDRA_INSTALL_DIR" ]; then
    for _g in "$HOME"/Applications/ghidra_*_PUBLIC(N/on); do
        GHIDRA_INSTALL_DIR="$_g"
    done
    unset _g
fi
if [ -n "$GHIDRA_INSTALL_DIR" ] && [ -d "$GHIDRA_INSTALL_DIR" ]; then
    export GHIDRA_INSTALL_DIR
    PATH="$PATH:$GHIDRA_INSTALL_DIR"
fi

# NSA Command-Line-Tools osx scripts, if checked out
[ -d "$HOME/Applications/Command-Line-Tools/scripts/osx" ] && PATH="$PATH:$HOME/Applications/Command-Line-Tools/scripts/osx"

# AFL++: set AFL_DIR to a source build, or install it on PATH
if [ -n "$AFL_DIR" ] && [ -x "$AFL_DIR/afl-fuzz" ]; then
    alias afl-clang-fast="$AFL_DIR/afl-clang-fast"
    alias afl-fuzz="$AFL_DIR/afl-fuzz"
elif [ -x "$HOME/Applications/AFLplusplus/afl-fuzz" ]; then
    alias afl-clang-fast="$HOME/Applications/AFLplusplus/afl-clang-fast"
    alias afl-fuzz="$HOME/Applications/AFLplusplus/afl-fuzz"
fi

# pyenv, only where it is installed
